...
class Documents
{
    ...
    public function run(){
        ...
        $this->view->assign('apiurl', $this->config['apiurl']);
        $this->view->assign('version', $this->config['version']);
        $this->view->assign('author', $this->config['author']);
        $this->view->assign('detail', $this->config['detail']);
    ...
    private function Item($class,$method)
    {
        ...
        return [
            ...
            'paramcode'=>isset($item['paramcode'])&&!empty($item['paramcode'])?$item['paramcode']:[],
            'returns'=>isset($item['returns'])&&!empty($item['returns'])?$item['returns']:[],
            'showcode'=>isset($item['showcode'])&&!empty($item['showcode'])?$item['showcode']:[],
        ];
    }
        ...
        foreach($methods as $value){
            if($value->class == $classname){
                if($access != null){
                    ...
                    switch($access){
                        case 'public':
                            if($methodAccess->isPublic()&&$value->name!='__construct')array_push($returnArr,$value->name);
                            break;
                        case 'protected':
    ...
    private function getData($res){
        $title = $description =  '';
        $param = $params = $return = $returns = $paramcode = $paramcodes = $showcode = $showcodes = array();
        ...
        return array('title'=>$title,'desc'=>$description,'params'=>$params,'paramcode'=>$paramcodes,'returns'=>$returns,'showcode'=>$showcodes);
    }
}
